...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        if type(width) == int and width > 0:
            ...
            raise ValueError('width wrong type')

        if type(height) == int and height > 0:
            self.height = height
        else:
            raise ValueError('height wrong type')

        if type(dirt_amount) == int and dirt_amount >= 0:
            self.dirt_amount = dirt_amount
        else:
            raise ValueError('dirt amount wrong type')
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        #check which tile we are on
        xtile = math.floor(pos.get_x())
        ytile = math.floor(pos.get_y())
        #reduce dirt of tile by capacity
        self.tiles[(xtile, ytile)] -= capacity
        if self.tiles[(xtile, ytile)] < 0 :
            self.tiles[(xtile, ytile)] = 0
    ...
    def get_num_cleaned_tiles(self):
        ...
        """
        tileDirt = list(self.tiles.values())
        numClean = tileDirt.count(0)
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        #start with random position and direction
        #make sure the starting position is valid (in room, no on furniture)
        self.position = Position(random.uniform(0,room.width), random.uniform(0,room.height))
        while not self.room.is_position_valid(self.position):
            self.position = Position(random.uniform(0,room.width), random.uniform(0,room.height))
...
class EmptyRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        """
        return Position(random.uniform(0,room.width), random.uniform(0,room.height))
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        newPos = curPos.get_new_position(self.get_robot_direction(), self.speed)
...
print ('avg time steps: ' + str(run_simulation(2, 1.0, 1, 20, 20, 1, 1.0, 1, StandardRobot)))
...
def show_plot_compare_strategies(title, x_label, y_label):
    ...
    for num_robots in num_robot_range:
        ...
        times1.append(run_simulation(num_robots, 1.0, 1, 10, 20, 3, 0.8, 20, StandardRobot))
