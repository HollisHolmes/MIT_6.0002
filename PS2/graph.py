...
class Digraph(object):
    ...
    def __str__(self):
        edge_strs = []
        #loops through lists of edges (out) for each node
        for edges in self.edges.values():
            #loops through edges for the current node
            for edge in edges:
    ...
    def add_edge(self, edge):
        ...
        in the  graph."""
        #check that both sourse and destination of edge are in the graphs
        #if either is not, raise a value error
        if not self.has_node(edge.get_source()) or not self.has_node(edge.get_destination()):
            raise ValueError('Node on edge does not exist')
        #bothn node are in the graph: and edge to list of edges in source node
        else:
