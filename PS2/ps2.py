...
#A: Travelling between buildings
#Where are the distances represented?
#A: Weights of the edges
...
def get_best_path(digraph, start, end, path, max_dist_outdoors, best_dist,
    ...
    if not digraph.has_node(digraph.get_node(start)) or not digraph.has_node(digraph.get_node(end)):
        raise ValueError('node aint in the graph homie')
    elif start == end:
        return path
    else:
        for child in digraph.get_edges_for_node(digraph.get_node(start)):
            child_name = child.get_destination().get_name()
            if child_name not in path[0]:
                distance = child.get_total_distance() + path[1]   # updating total distance.
                outdoors = child.get_outdoor_distance() + path[2]   # updating outdoor distance.
                new_path = [path[0], distance, outdoors]

                new2_path = get_best_path(digraph, child_name, end, new_path, max_dist_outdoors, best_dist,
                                  best_path)
